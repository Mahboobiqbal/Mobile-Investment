...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white rounded-lg px-4 py-3 shadow-lg border border-slate-200">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="card p-5 h-32 animate-pulse">
              ...
              <div className="h-8 w-20 rounded bg-slate-100 mt-3" />
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div className="card-hover p-5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Active Investors</span>
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600">
        ...
        <div className="card-hover p-5">
          ...
          <p className="text-2xl font-bold tracking-tight mt-3 text-slate-900">
            <span className="text-sm font-medium text-slate-400 mr-0.5">Rs.</span>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div className="card p-5 xl:col-span-2">
          <div className="flex flex-wrap items-start sm:items-center justify-between gap-2 mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">14-Day Activity</h2>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Transaction Status</h2>
            ...
            {transactionPieData.map(item => (
              <div key={item.name} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-semibold text-slate-900">{item.value.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
            ...
            {monthlyChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyChartData}>
                  ...
                  <YAxis tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Area type="monotone" dataKey="users" stroke={COLORS.emerald} strokeWidth={2} fill="url(#userGrowth)" dot={{ r: 3, fill: COLORS.emerald, strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 5, fill: COLORS.emerald, strokeWidth: 2, stroke: '#fff' }} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex h-full items-center justify-center">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Deposits vs Withdrawals</h2>
                ...
                <RechartsScatterChart>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="bg-white rounded-lg px-4 py-3 shadow-lg border border-slate-200">
                          <p className="text-xs font-medium text-slate-500 mb-1">{d.date}</p>
                  ...
                  />
                  <Scatter data={scatterData} fill={COLORS.emerald} stroke={COLORS.emeraldLight} strokeWidth={1} />
                </RechartsScatterChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex h-full items-center justify-center">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Financial Flow</h2>
          ...
          <div className="space-y-5">
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm font-semibold text-emerald-600">Rs. {data.transactions.totalDeposits.toLocaleString()}</span>
              </div>
            ...
            <div>
              <div className="flex justify-between mb-2">
                ...
                <span className="text-sm font-semibold text-rose-600">Rs. {data.transactions.totalWithdrawals.toLocaleString()}</span>
              </div>
              <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                <div
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Top Investors</h2>
          ...
          <div className="space-y-2">
            {data.topUsers.map((user, i) => (
              <div key={user._id} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 p-3 transition-all hover:bg-slate-100 border border-slate-100">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="relative shrink-0">
                    <div className={`flex h-9 w-9 items-center justify-center rounded-lg text-xs font-semibold text-white ${
                      i === 0 ? 'bg-emerald-600' :
                      i === 1 ? 'bg-slate-600' :
                      i === 2 ? 'bg-amber-600' :
                      'bg-slate-400'
                ...
                <span className="text-sm font-semibold text-emerald-600 shrink-0">Rs. {user.currentBalance.toLocaleString()}</span>
              </div>
            ))}
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Transaction Overview</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg bg-blue-50 p-4 border border-blue-100">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">Deposits</span>
                <Wallet className="h-4 w-4 text-blue-500" />
              </div>
              <p className="text-lg font-bold text-slate-900">Rs. {data.transactions.totalDeposits.toLocaleString()}</p>
              <p className="text-xs text-slate-500 mt-1">Total deposited</p>
            </div>
            <div className="rounded-lg bg-rose-50 p-4 border border-rose-100">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">Withdrawals</span>
                <Banknote className="h-4 w-4 text-rose-500" />
              </div>
              <p className="text-lg font-bold text-slate-900">Rs. {data.transactions.totalWithdrawals.toLocaleString()}</p>
